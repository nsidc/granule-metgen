...
def test_bounding_rectangle_from_attrs_with_valid_latlon():
    ...
    result = netcdf_reader.bounding_rectangle_from_attrs(mock_netcdf)
...
def test_bounding_rectangle_from_attrs_missing_latlon():
    ...
    with pytest.raises(Exception) as exc_info:
        netcdf_reader.bounding_rectangle_from_attrs(mock_netcdf)
...
def test_spatial_values_geodetic_prefers_geospatial_bounds():
    ...
    with (
        patch("nsidc.metgen.readers.netcdf_reader.find_grid_mapping_var"),
        patch("nsidc.metgen.readers.netcdf_reader.crs_transformer"),
        patch("nsidc.metgen.readers.netcdf_reader.pixel_padding"),
        patch(
            "nsidc.metgen.readers.netcdf_reader.find_coordinate_data_by_standard_name"
        ),
    ):
        result = netcdf_reader.spatial_values(
            mock_netcdf, mock_config, constants.GEODETIC
        )

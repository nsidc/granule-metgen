...
class TestExtractMetadata:
    ...
    def test_uses_premet_temporal_override(self, test_config):
        ...
        temporal_override = [
            {"start": "2021-01-01T00:00:00Z", "end": "2021-01-01T23:59:59Z"}
        ]

        with pytest.raises(NotImplementedError):
            extract_metadata(
                "/test/snowex_file.csv", temporal_override, [], test_config, "GEODETIC"
            )
    ...
    def test_uses_spatial_file_override(self, test_config):
        ...
        with pytest.raises(NotImplementedError):
            extract_metadata(
                "/test/snowex_file.csv", [], spatial_override, test_config, "GEODETIC"
            )

...
class IntegrationTestRunner:
    ...
    BASE_URL = "http://staging-http.apps.int.nsidc.org/staging/SAMPLE_DATA/DUCk_project/"
    ...
    def download_collection_data(self, collection_name: str, output_dir: Path, 
        ...
        if not force_download and output_dir.exists():
            ...
            if existing_files:
                logger.info(f"Found {len(existing_files)} existing files for {collection_name}, skipping download")
    ...
    def _download_subdirectory(self, subdir_url: str, output_dir: Path, 
                              force_download: bool = False) -> List[Path]:
    ...
    def run_metgenc(self, ini_file: Path, environment: str = "uat") -> Tuple[bool, Optional[str]]:
    ...
    def validate_output(self, config_file: Path, output_dir: Path) -> Tuple[bool, List[str]]:
        ...
        if cnm_dir.exists():
            ...
            if cnm_files:
                ...
                cmd = ["metgenc", "validate", "--config", str(config_file), "--type", "cnm"]
                try:
        ...
        if ummg_dir.exists():
            ...
            if ummg_files:
                ...
                cmd = ["metgenc", "validate", "--config", str(config_file), "--type", "ummg"]
                try:
                    result = subprocess.run(cmd, capture_output=True, text=True, timeout=60)
                    if result.returncode != 0:
    ...
    def run_test(self, collection_name: str, environment: str = "uat", 
                 keep_files: bool = False, force_download: bool = False) -> bool:
        ...
        try:
            ...
            downloaded_files = self.download_collection_data(collection_name, data_dir, force_download)
...
"""
    ...
    parser.add_argument(
        "collection",
        help="Collection name to test (or 'all' for all collections)"
    )
    parser.add_argument(
        "-e", "--environment",
        default="uat",
    ...
    if not available:
        ...
        logger.error("Please ensure INI files are present in tests/integration/configs/")
        sys.exit(1)

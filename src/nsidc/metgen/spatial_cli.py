...
def compare_polygons(collection, number, provider, token_file, output, granule):
    ...
    if token_file:
        ...
        try:
            ...
            click.echo(f"Warning: Could not read token file {token_path}: {e}", err=True)
...
@click.option('--format', 'file_format', 
    ...
    try:
        ...
        if check_coverage:
            ...
            if 'lon' not in df.columns or 'lat' not in df.columns:
                click.echo("Error: Points file must have 'lon' and 'lat' columns", err=True)
                sys.exit(1)
            ...
            click.echo(f"Coverage analysis:")

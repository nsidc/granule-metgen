...
class PolygonGenerator:
    ...
    def create_flightline_polygon(self, lon, lat, method='adaptive_beam', 
        ...
        if iterative_simplify and polygon is not None:
            from iterative_simplification import iterative_simplify_polygon
            
            # Sample data points for coverage calculation
            sample_size = min(10000, len(lon))
            sample_indices = np.random.choice(len(lon), sample_size, replace=False)
            data_points = points[sample_indices]
    ...
    def _create_beam_polygon(self, points, buffer_distance):
        """Create polygon using beam method (buffer with connections)."""
        # Subsample if too many points
        if len(points) > 50000:

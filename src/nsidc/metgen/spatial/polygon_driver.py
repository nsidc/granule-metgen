...
class PolygonComparisonDriver:
    ...
    def process_specific_granule(self, short_name, granule_ur, provider=None, 
        ...
        try:
            ...
            if not entries:
                print(f"Error: Could not find granule with producerGranuleId '{granule_ur}' in collection {short_name}")
                return
            
            if len(entries) > 1:
                print(f"Warning: Found {len(entries)} granules matching producerGranuleId '{granule_ur}'")
            
            target_granule = entries[0]
            print(f"Found granule: {target_granule.get('title', 'Unknown')}")
            
            # Process the granule
            result = self.process_granule(target_granule, collection_dir, data_extensions)
    ...
    def process_granule(self, granule_entry, output_dir, data_extensions):
        ...
        try:
            ...
            if cmr_coverage > 0:
                method_params['min_coverage'] = max(cmr_coverage, method_params.get('min_coverage', 0.90))
                print(f"  Using minimum coverage constraint: {method_params['min_coverage']:.1%}")
    ...
    def load_data_points(self, data_file):
        ...
        """
        try:
            print(f"    Attempting to load file with suffix: {data_file.suffix.lower()}")
            if data_file.suffix.lower() in ['.txt', '.csv']:
                ...
                for i, line in enumerate(first_lines):
                    if line.strip().startswith('#'):
                        ...
                        if any(delimiter in line for delimiter in [',', '\t']) or len(line.split()) > 3:
                            last_comment_idx = i
                ...
                if last_comment_idx >= 0:
                    ...
                    print(f"    Found header at line {header_idx + 1} (last comment before data)")
                else:
                ...
                print(f"    Header: {header_line.strip() if header_line else 'No header found'}")
                    ...
                    if header_idx < len(first_lines) and first_lines[header_idx].strip().startswith('#'):
                        ...
                        if has_comma:
                            ...
                            data = pd.read_csv(data_file, sep=',', engine='python', 
                                             ...
                                             skiprows=header_idx+1, skipinitialspace=True)
                        elif has_tab:
                            data = pd.read_csv(data_file, sep='\t', engine='python', 
                                             comment='#', names=columns, 
                        ...
                        if has_comma:
                            ...
                            data = pd.read_csv(data_file, sep='\t', engine='python', comment='#')
                        else:
                            print(f"    Reading whitespace-delimited format: {data_file}")
                            data = pd.read_csv(data_file, sep=r'\s+', engine='python', comment='#')
                ...
                if 'data' in locals() and data is not None:
                    ...
                    if lon_col and lat_col:
                        ...
                        print(f"    Found {valid_count} valid coordinates out of {len(lon)}")
                        ...
                        print(f"    Column details: {[(col, type(col)) for col in data.columns]}")
                        return None, None
    ...
    def create_granule_summary(self, output_dir, granule_ur, lon, lat,
        ...
        left_table_data = [
            ...
            ['Area', f'{metrics["cmr_area_deg2"]:.6f}°²', f'{metrics["generated_area_deg2"]:.6f}°²'],
            ['Vertices', f'{metrics["cmr_vertices"]}', f'{metrics["generated_vertices"]}'],
            ['Data Coverage', f'{metrics.get("cmr_data_coverage", 0):.1%}', f'{metrics.get("generated_data_coverage", 0):.1%}'],
        ]
        
        # Add non-data coverage if available
        if 'cmr_non_data_coverage' in metrics:
            left_table_data.append(['Non-data Coverage', f'{metrics.get("cmr_non_data_coverage", 0):.1%}', f'{metrics.get("generated_non_data_coverage", 0):.1%}'])
        
        # Right column - Optimization Goals
        right_table_data = [
            ['Goal', 'Status'],
            ['Primary Goals', ''],
            ['100% Data Coverage', '✓' if metrics.get("generated_data_coverage", 0) >= 1.0 else f'✗ ({metrics.get("generated_data_coverage", 0):.1%})'],
            ['Vertices ≤ CMR', '✓' if metrics["generated_vertices"] <= metrics["cmr_vertices"] else f'✗ ({metrics["generated_vertices"]} > {metrics["cmr_vertices"]})'],
            ['Area ≤ CMR', '✓' if metrics["area_ratio"] <= 1.0 else f'✗ ({metrics["area_ratio"]:.2f}x)'],
            ['', ''],
            ['Generation Info', ''],
            ['Method', metadata.get('method', 'unknown')],
            ['Adaptive Buffer', f'{metadata.get("adaptive_buffer", "N/A"):.1f} m' if isinstance(metadata.get("adaptive_buffer"), (int, float)) else 'N/A'],
        ]
        ...
        for row_idx in range(len(left_table_data)):
            ...
            if cell_key == 'IoU' and metrics['iou'] >= 0.8:
                ...
                left_table[(row_idx, 1)].set_facecolor('lightgreen')
            elif cell_key == 'Area Ratio' and (metrics['area_ratio'] < 0.5 or metrics['area_ratio'] > 2.0):
                left_table[(row_idx, 1)].set_facecolor('lightcoral')
            elif cell_key == 'CMR Coverage' and metrics['cmr_covered_by_generated'] >= 0.9:
                left_table[(row_idx, 1)].set_facecolor('lightgreen')
            elif cell_key == 'CMR Coverage' and metrics['cmr_covered_by_generated'] < 0.9:
                left_table[(row_idx, 1)].set_facecolor('lightcoral')
        ...
        ax_combined.set_title('Metrics & Generation Summary', fontweight='bold', fontsize=14, pad=20)

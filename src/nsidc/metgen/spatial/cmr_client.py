...
class CMRClient:
    ...
    def query_granules(self, short_name, provider=None, limit=2000, 
                      temporal=None, bounding_box=None, sort_key=None,
                      page_size=100):
        """
    ...
    def get_random_granules(self, short_name, provider=None, count=10):
        ...
        # For small collections, just get all and sample
        if total_hits <= count * 10:
        ...
        if (end_dt - start_dt).days < 14:
            return self.query_granules(
                short_name, provider=provider, limit=count
            )
        ...
        while len(random_granules) < count and attempts < max_attempts:
            ...
            # Query small window around random date
            window_start = (random_date - timedelta(days=1)).isoformat() + "Z"
            window_end = (random_date + timedelta(days=1)).isoformat() + "Z"
            
            granules = self.query_granules(
                short_name, provider=provider, limit=1,
        ...
        if len(random_granules) < count:
            recent = self.query_granules(
                short_name, provider=provider, 
                limit=count - len(random_granules),
                sort_key="-start_date"
            )
            for g in recent:
                g_id = g.get('id', '')
...
class UMMGParser:
            ...
            if len(points) >= 3:
                ...
                feature = {
                    ...
                    "properties": {
                        ...
                        "type": "GPolygon",
                        "index": i,
                        "granule_ur": granule_ur or umm_json.get("GranuleUR", "Unknown")
                    }
                }
    ...
    def extract_data_urls(umm_json):
        ...
        for info in archive_info:
            url = info.get("DirectDistributionInformation", {}).get("S3BucketAndObjectPrefixNames", [""])[0]
            if url:
    ...
    def find_data_file(urls, extensions=['.TXT', '.txt', '.h5', '.HDF5', '.nc']):
        ...
        # Return first URL if no extension match
...
class PolygonComparator:
    ...
    def compare(cmr_geojson, generated_geojson, data_points=None):
        ...
        generated_gdf = gpd.GeoDataFrame.from_features(generated_geojson['features'])
        
        # Get geometries
        cmr_geom = cmr_gdf.unary_union
        generated_geom = generated_gdf.unary_union
        ...
        metrics['cmr_covered_by_generated'] = intersection / cmr_area if cmr_area > 0 else 0
        metrics['generated_covered_by_cmr'] = intersection / generated_area if generated_area > 0 else 0
        
        # Vertex counts
        if hasattr(cmr_geom, 'exterior'):
            metrics['cmr_vertices'] = len(cmr_geom.exterior.coords) - 1
        else:
            # MultiPolygon case
            metrics['cmr_vertices'] = sum(len(p.exterior.coords) - 1 for p in cmr_geom.geoms)
        
        if hasattr(generated_geom, 'exterior'):
            metrics['generated_vertices'] = len(generated_geom.exterior.coords) - 1
        else:
            metrics['generated_vertices'] = sum(len(p.exterior.coords) - 1 for p in generated_geom.geoms)
        
        # Distance metrics
        metrics['hausdorff_distance'] = cmr_geom.hausdorff_distance(generated_geom)
        metrics['centroid_distance'] = cmr_geom.centroid.distance(generated_geom.centroid)
        ...
        if data_points is not None and len(data_points) > 0:
            ...
            metrics['cmr_data_coverage'] = cmr_coverage_count / total_points if total_points > 0 else 0
            metrics['generated_data_coverage'] = generated_coverage_count / total_points if total_points > 0 else 0
            metrics['data_coverage_ratio'] = (metrics['generated_data_coverage'] / 
                                             metrics['cmr_data_coverage'] if metrics['cmr_data_coverage'] > 0 else 0)
            ...
            print(f"    Using adaptive radius: {adaptive_radius*111000:.0f}m (2% of {smaller_dimension*111000:.0f}m bounding box)")
            ...
            def estimate_coverage_with_params(polygon, radius, num_samples):
                ...
                if len(random_points) >= 1000:  # Only for final calculations
                    print(f"        Sampled {len(random_points)} points: {points_with_data} have data, "
                          f"{len(random_points) - points_with_data} are empty")
                    print(f"        Data coverage: {data_coverage:.1%}, Non-data coverage: {non_data_coverage:.1%}")
            ...
            def find_converged_coverage(polygon, convergence_threshold=0.01):
                ...
                for samples in sample_sizes:
                    ...
                    if prev_coverage is not None:
                        ...
                        if abs(coverage - prev_coverage) < convergence_threshold:
                            ...
                            converged = True
                            print(f"        Converged at {samples} samples (change < 1%)")
                            break
                ...
                final_coverage = estimate_coverage_with_params(polygon, radius, best_samples)
            ...
            print(f"      CMR polygon: converged at {cmr_samples} samples, {cmr_radius*111000:.0f}m radius")
            
            gen_coverage, gen_radius, gen_samples = find_converged_coverage(generated_geom)
            print(f"      Generated polygon: converged at {gen_samples} samples, {gen_radius*111000:.0f}m radius")
            ...
            metrics['cmr_coverage_params'] = {'radius_deg': cmr_radius, 'samples': cmr_samples}
            metrics['generated_coverage_params'] = {'radius_deg': gen_radius, 'samples': gen_samples}
...
def sanitize_granule_ur(granule_ur):
    ...
    safe_name = re.sub(r'[<>:"/\\|?*]', '_', granule_ur)
